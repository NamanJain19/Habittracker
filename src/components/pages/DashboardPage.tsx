        ...
        <div className="max-w-[100rem] mx-auto space-y-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            >
              <div className="flex items-center gap-4">
                <div className="p-3 rounded-xl bg-gradient-to-br from-accent-teal/20 to-accent-teal/10 border border-accent-teal/30">
                  <CheckCircle2 className="w-6 h-6 text-accent-teal" />
                </div>
                <div>
                  <div className="font-heading text-3xl font-bold text-accent-teal">
            ...
            >
              <div className="flex items-center gap-4">
                <div className="p-3 rounded-xl bg-gradient-to-br from-accent-purple/20 to-accent-purple/10 border border-accent-purple/30">
                  <Target className="w-6 h-6 text-accent-purple" />
                </div>
                <div>
                  <div className="font-heading text-3xl font-bold text-accent-purple">
                    {activeGoals}
                  </div>
                  <div className="font-paragraph text-sm text-light-foreground/60">
                    Active Goals
            ...
            >
              <div className="flex items-center gap-4">
                <div className="p-3 rounded-xl bg-gradient-to-br from-accent-teal/20 to-accent-purple/20 border border-accent-teal/30">
            ...
            {isLoading ? null : (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {trackerCards.map((card, index) => (
                  ...
                  >
                    <Link to={card.link}>
                      <div className={`backdrop-blur-xl bg-gradient-to-br ${card.color} border ${card.borderColor} rounded-2xl p-6 hover:scale-105 transition-transform duration-300 group`}>
                        <div className="flex items-center justify-between mb-4">
                          <div className={`p-3 rounded-xl bg-gradient-to-br ${card.color} border ${card.borderColor}`}>
                            <card.icon className={`w-6 h-6 ${card.textColor}`} />
                          </div>
                          <div className={`font-heading text-4xl font-bold ${card.textColor}`}>
                            {card.count}
                          </div>
                        </div>
          ...
          >
            <div className="flex items-center gap-3 mb-6">
              ...
              <h2 className="font-heading text-2xl font-bold text-light-foreground">
                Recent Activity
              </h2>
            </div>
              ...
              <div className="space-y-4">
                {habits.slice(0, 3).map((habit) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <CheckCircle2 className={`w-5 h-5 ${habit.isCompleted ? 'text-accent-teal' : 'text-light-foreground/30'}`} />
                      <div>
                        <div className="font-paragraph text-sm font-bold text-light-foreground">
                          {habit.habitName}
                        </div>
                ...
                {habits.length === 0 && goals.length === 0 && (
                  <div className="text-center py-12">
                    <Brain className="w-12 h-12 text-light-foreground/30 mx-auto mb-4" />
